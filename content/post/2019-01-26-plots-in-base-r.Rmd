...
Hist can also be used to plot densities.  Below the y axis has been transformed to measure the probabilty density of sepal lengths.  The red line shows a smoothed kernal density in contrast to a theoretical normal curve in green.  
```{r plot1b}
hist(Sepal.Length,
     freq = FALSE,
...
##pairs()  
If you are analizing datasets with 2 or more independent variables, then the pairs function will provide you with a high level view of the feature correlations.  
```{r plot4}
pairs(iris)
```
...
```
Alot of custimization can be done with base R graphics.